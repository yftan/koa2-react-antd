/**
 * Created by Franklin on 2017/3/9.
 */
...
MapInitControl.init = function () {
    ...
    window.marker = new BMap.Marker(new BMap.Point(118, 32), {icon: window.icnLocateIcon});
    window.setPoint = function (pt) {
        map.removeOverlay(window.marker);
        marker = new BMap.Marker(pt, {icon: window.icnLocateIcon});
    ...
    var mapTypeControl = new BMap.MapTypeControl({type : BMAP_MAPTYPE_CONTROL_MAP, mapTypes : [BMAP_NORMAL_MAP, BMAP_SATELLITE_MAP], anchor: BMAP_ANCHOR_TOP_LEFT});
...
//鼠标点击事件
MapInitControl.onClick = function (e) {
    window.setPoint(e.point);
};

MapInitControl.TransBlack2HTML = function (black) {
...
MapInitControl.TransTree2HTML = function (tree) {
    var treeStatus = null;
    switch (tree)
    {
        case 0 :
...
MapInitControl.TransPipe2HTML = function (pipe) {
    var pipeStatus = null;
    switch (pipe)
    {
        case 0 :
